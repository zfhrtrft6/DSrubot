...
# Точные названия каналов для отслеживания творчества
...
async def on_ready():
    init_db()
    print(f"=== Робот {bot.user.name} запущен! Канал 'творчество 18' в режиме иммунитета. ===")
    check_old_members.start()
    clear_expired_warns.start()


# === Фоновое сгорание варнов через 48 часов ===
...
async def clear_expired_warns():
    ...
    for row in rows:
        ...
        if now - warn_time >= timedelta(hours=48):
            ...
            print(f"Варн ID {warn_id} автоматически сгорел.")
            
    conn.commit()
    conn.close()


# === 1. АВТО-ВЫДАЧА РОЛЕЙ ПРИ ВХОДЕ ===
@bot.event
...
# === 2. АВТО-ПРОВЕРКА НА РОЛЬ «СТАРЕЦ» ===
@tasks.loop(hours=1)
...
async def apply_warning(message, member, count_to_add, reason):
    ...
    if active_warns == 1:
        await message.channel.send(
            f"⚠️ {member.mention}, тебе выдано предупреждение! **Причина:** {reason}. Варн сгорит через 48ч. Всего активных варнов: 1/4.",
            delete_after=15
        )
    elif active_warns == 2:
        try:
            await member.timeout(timedelta(minutes=10), reason=reason)
            await message.channel.send(f"🤫 {member.mention} отправлен в мут на **10 минут** за 2-й активный варн! **Причина:** {reason}.")
        except discord.Forbidden:
            pass
    elif active_warns == 3:
        try:
            await member.timeout(timedelta(days=1), reason=reason)
            await message.channel.send(f"⛔ {member.mention} отправлен в мут на **24 часа** за 3-й активный варн! **Причина:** {reason}.")
        except discord.Forbidden:
            pass
    elif active_warns >= 4:
        ...
        if bot_choice == "mute":
            try:
                ...
                await message.channel.send(f"⚖️ Бот принял решение: {member.mention} получает супер-мут на **50 часов** за 4 нарушения правил!")
            except discord.Forbidden:
                pass
        elif bot_choice == "ban":
            try:
...
# === 3. АВТО-МОДЕРАЦИЯ И СБОР ТВОРЧЕСТВА ===
@bot.event
async def on_message(message):
    ...
    # ---- АВТО-МОДЕРАЦИЯ ТЕКСТА (ПРОПУСКАЕТ КАНАЛ ТВОРЧЕСТВО 18) ----
    if channel_name != "творчество 18":
        ...
        if channel_name != "творчество":  # В обычном творчестве 18+ тоже нельзя, только в творчестве 18
            if any(word in text_lower for word in nsfw_words):
                await apply_warning(message, member, 1, "18+ контент вне творческой 18+ зоны")
                return

    # ---- АВТО-ПОДСЧЕТ АРТОВ И АНИМАЦИЙ (РАБОТАЕТ ВО ВСЕХ ТВОРЧЕСКИХ КАНАЛАХ) ----
